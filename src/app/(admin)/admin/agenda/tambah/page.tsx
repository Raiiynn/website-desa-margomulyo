...
import { createAgenda } from '@/server/admin-actions';
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-serif text-2xl font-bold text-navy-900">Tambah Agenda</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-border bg-white p-6 space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <AdminFormField label="Judul Kegiatan" name="title" required placeholder="Rakor Pamong Bulanan" />
          <AdminFormField label="Slug (URL)" name="slug" required placeholder="rakor-pamong-bulanan" />
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <AdminFormField label="Label Kategori" name="label" placeholder="Rakor Pamong" helpText="Opsional." />
          <AdminFormField label="Lokasi" name="location" placeholder="Balai Kalurahan Margomulyo" />
        </div>

        <AdminTextAreaField label="Deskripsi" name="description" placeholder="Deskripsi kegiatan..." rows={3} />

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          <AdminFormField label="Mulai" name="startsAt" type="datetime-local" required />
          <AdminFormField label="Selesai" name="endsAt" type="datetime-local" helpText="Opsional." />
